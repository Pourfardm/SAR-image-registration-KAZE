...
buildingDir = fullfile('C:\Users\HP\Pictures\Database Images\Our SAR database test\test28');
% buildingDir = fullfile('F:\MATLAB\descriptors\stitch\imageStitchMatlab\imageStitchMatlab\img\sar_paper');%good
...
% points = detectMSERFeatures(grayImage_prev);
points = detectBRISKFeatures(grayImage_prev);
% points = detectFASTFeatures(grayImage_prev);
...
%     points = detectMSERFeatures(grayImage_prev2);
    points = detectBRISKFeatures(grayImage_prev);
%     points = detectFASTFeatures(grayImage_prev);
    ...
    figure(5),showMatchedFeatures_separate(grayImage_prev2,grayImage_prev,matchedPoints,matchedPointsPrev);
    legend('matched points 1','matched points 2');
